...
Deploying to Base follows the same process as Ethereum mainnet. The contracts in `blockchain/contracts/` are fully compatible. Gas costs on Base are significantly lower.
...
Attestations on Base cost a fraction of mainnet gas while maintaining the same verifiability.
