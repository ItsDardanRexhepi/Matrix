...
class CreatorPlatformsService:
    ...
    async def mint_sound(self, **params: Any) -> dict:
        """Stub for mint_sound. Returns not_deployed until the adapter is wired."""
        return not_deployed_response(
            self.service_name,
            extra={"method": "mint_sound", "params": params},
        )

    async def publish_mirror_post(self, **params: Any) -> dict:
        """Stub for publish_mirror_post. Returns not_deployed until the adapter is wired."""
        return not_deployed_response(
            self.service_name,
            extra={"method": "publish_mirror_post", "params": params},
        )

    async def publish_paragraph_post(self, **params: Any) -> dict:
        """Stub for publish_paragraph_post. Returns not_deployed until the adapter is wired."""
        return not_deployed_response(
            self.service_name,
            extra={"method": "publish_paragraph_post", "params": params},
        )
