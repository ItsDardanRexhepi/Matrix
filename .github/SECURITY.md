...
One default is documented rather than a finding: without the private security package installed, the Morpheus gate is an OBSERVE no-op that allows every action it evaluates (see `SECURITY_STUB.md` for what still applies). A way past any public check that does still apply — among them the per-agent tool boundary, the seam-level refusals on platform-signed actions, the Rexhepi (URF) gate, the Glasswing audit block on contract deployments, and fail-closed owner verification and OTP — is a finding, and so is a way to make a deployment that has the package installed behave as if it did not.
