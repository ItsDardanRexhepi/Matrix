...
def _build_mock_server(config):
    ...
    server.apple_users = MagicMock()
    server.apple_users.initialize = AsyncMock()
    server.apple_users.touch = AsyncMock(return_value=True)
    server.apple_users.link_wallet = AsyncMock()
    server.apple_users.wallet_for = MagicMock(return_value="")
