...
behaves, and only test_config_validation and test_readiness owned any of them:
  ...
  MATRIX_ENV=production      — every GatewayServer construction becomes a
                                 ...
                                 that build a server.
  MATRIX_API_KEY           — `api_key = gw.get("api_key") or os.environ[...]`
  ...
  MATRIX_OTP_PEPPER          — with the private package importable, its
...
MATRIX_* / MATRIX_* variable cleared (by prefix, so a switch added later is
...
left all 547 sweep cases erroring under MATRIX_ENV=production.
...
.env holding MATRIX_ENV=production gave test_apns_env_override followed by
...
def test_a_hostile_environment_does_not_reach_a_test(tmp_path):
    ...
    (tmp_path / ".env").write_text(
        "MATRIX_ENV=production\nMATRIX_API_KEY=dotenv-developer-key\n")

    env = dict(os.environ)
    env.update({
        "MATRIX_ENV": "production",
        "MATRIX_API_KEY": "ambient-developer-key",
        "MATRIX_ALLOW_TEST_MINT": "1",
        "MATRIX_OTP_PEPPER": "ambient-pepper",
