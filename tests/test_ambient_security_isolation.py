...
scope and module scope) to pass. Without the conftest fixture the probe fails,
...
def test_a_hostile_environment_does_not_reach_a_test(tmp_path):
    ...
    proc = subprocess.run(
        [sys.executable, "-m", "pytest", "tests/ambient_isolation_probe.py",
    ...
    assert "2 passed" in proc.stdout, tail
