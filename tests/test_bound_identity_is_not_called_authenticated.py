...
@pytest.mark.asyncio
async def test_the_chat_route_refuses_a_body_wallet_from_a_non_operator():
    """MERGE of cd/gateway and cd/routes-honesty changed what is true here.

    routes-honesty wrote this test to pin the behaviour it had documented: /chat
    threaded whatever `wallet` the body carried, for anyone. cd/gateway then made
    the chat entrances derive identity from the presented session and consult a
    caller-written field ONLY for an operator request. The stricter code wins, so
    this asserts the new truth rather than the old one:

      * a session caller's body `wallet` is ignored — and, unlike the version
        this replaces, the session's own linked wallet IS what gets threaded;
      * the asserted path still exists for an operator, and is named as asserted
        in docs/api-reference.md rather than called authenticated.
    """
    ...
    async with TestClient(TestServer(server.create_app())) as client:
        ...
        threaded = context.metadata["user_context"].get("wallet_address", "")
        assert threaded != CHAT_BODY_WALLET, (
            "a body-supplied wallet was threaded for a caller holding a session")
        assert threaded == LINKED_WALLET, (
            "the session's linked wallet is what the chat entrances derive")
