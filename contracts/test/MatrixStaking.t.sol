...
/// @title MatrixStaking.t.sol
/// @notice Unit tests for the ETH staking contract.
contract MatrixStakingTest is Test {
    ...
    function test_Rewards_ComeOnlyFromTheReserve_AndAreOwedWhenItIsEmpty() public {
        MatrixStaking bare = new MatrixStaking(feeRecipient);   // no fundRewards
        vm.deal(alice, 1 ether);
        vm.prank(alice);
    ...
    function test_OwedRewards_ArePaidWhenTheReserveIsFunded() public {
        MatrixStaking bare = new MatrixStaking(feeRecipient);
        vm.deal(alice, 1 ether);
        vm.prank(alice);
        bare.stake{value: 1 ether}();
        vm.warp(block.timestamp + 30 days);
