...
contract OpenMatrixInsuranceTest is Test {
    ...
    function _buy(address who, uint256 coverage) internal returns (uint256 id) {
        uint256 premium = insurance.calculatePremium(coverage, OpenMatrixInsurance.RiskTier.Low);
        vm.deal(who, who.balance + premium);
        vm.prank(who);
        id = insurance.purchasePolicy{value: premium}(coverage, OpenMatrixInsurance.RiskTier.Low, TRIGGER);
    }
    ...
    function _buyWith(address who, uint256 coverage, bytes32 cond) internal returns (uint256 id) {
        ...
        vm.deal(who, who.balance + premium);
        vm.prank(who);
        id = insurance.purchasePolicy{value: premium}(coverage, OpenMatrixInsurance.RiskTier.Low, cond);
    }

    function test_Purchase_BeyondBacking_Reverts() public {
        // The pool holds 50 ETH (setUp). 40 ETH of coverage sells; 20 more does not.
        _buy(alice, 40 ether);
        uint256 premium = insurance.calculatePremium(20 ether, OpenMatrixInsurance.RiskTier.Low);
        vm.deal(alice, premium);
        vm.prank(alice);
        vm.expectRevert("Coverage exceeds backing");
        insurance.purchasePolicy{value: premium}(20 ether, OpenMatrixInsurance.RiskTier.Low, TRIGGER);
    }
    ...
    function test_Purchase_PerHolderCap() public {
        ...
        uint256 premium = insurance.calculatePremium(0.01 ether, OpenMatrixInsurance.RiskTier.Low);
        vm.deal(alice, premium);
        vm.prank(alice);
        vm.expectRevert("Too many active policies");
        insurance.purchasePolicy{value: premium}(0.01 ether, OpenMatrixInsurance.RiskTier.Low, TRIGGER);
        // Expiry frees a slot.
        vm.warp(block.timestamp + 31 days);
