...
- **Attestations.** An attestation a model-facing tool writes because a user asked for it (the `eas` tool's `attest` and `batch_attest`, agent identity, identity, insurance, gaming, cross-border, IP, securities and supply-chain records) is metered like any platform-signed operation, under its `<capability>.<method>` name (for example `eas_manager.attest`).
- **Not metered.** Writes listed in `UNMETERED_PLATFORM_OPERATIONS` are signed and paid by the platform without counting against any cap: attestations reached from the services layer (the dispatcher's record of each capability call, records written after another operation, and the `create_attestation` and `batch_attest` capabilities, whose queued batches are signed later with no caller to attribute), revoking an attestation through the services layer, and moving platform revenue to its treasury multisig. A single batch request may hold at most 20 attestations.
