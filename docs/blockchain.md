...
- **Attestations.** An attestation written because a user asked for it is metered like any platform-signed operation, under its `<capability>.<method>` name: the `eas` tool's `attest` and `batch_attest` and the tool-layer records (agent identity, identity, insurance, gaming, cross-border, IP, securities, supply chain) under names such as `eas_manager.attest`, and the `create_attestation`, `batch_attest` and `revoke_attestation` capabilities under `attestation.attest`, `attestation.batch_attest` and `attestation.revoke`. For a capability attestation that is queued rather than time-critical, the allowlist and the identity requirement are applied when it is queued and the cap when its batch signs, against the identity it was queued under; a refusal at that later point is recorded in the batch result and logged. A single batch request may hold at most 20 attestations.
- **Not metered.** Writes listed in `UNMETERED_PLATFORM_OPERATIONS` are signed and paid by the platform without counting against any cap: the platform's own records (the dispatcher's record of each capability call, and records a service writes after another operation — a conversion deploy, a claim outcome, a royalty), the sponsorship accounting path itself, and moving platform revenue to its treasury multisig. No attestation a caller composes is on that list.
