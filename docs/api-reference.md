...
**Identity is derived from a session when one is presented.** On a request carrying a wallet session, the caller's identity is the session's subject — the wallet linked to the Apple user when one exists, else `apple:<sub>` or the SIWE address — and nothing in the request overrides it on the routes the security middleware binds (every `POST /api/v1/*`). **`/chat` is the exception, session or not.** The identity it threads to the agents' tools — and that the tools record as the actor — is the request body's own `wallet` (or `wallet_address`) field; the session's subject and the wallet linked to it are not consulted, and with no such field the threaded identity is empty. `/chat/stream` and `/ws` thread none at all; `/bridge/v1/chat` threads the address of a wallet session linked to that `session_id` through the bridge's own link step, which reads it from the server-side session record and ignores the body. With no session (an operator-key request, or a gateway running with auth off) the identity is asserted, not derived: the `X-Wallet-Address` header, else a `wallet`, `from`, `sender` or `account` body field or `params.from`, as the caller wrote it. Routes that record or check "the caller" (for example `/api/v1/capabilities/{id}/invoke`, `/api/v1/insurance/claim`, `/api/v1/paymaster/sign`) use that asserted value. A wallet proven by `POST /auth/verify` while holding an Apple session is linked to that Apple user.
