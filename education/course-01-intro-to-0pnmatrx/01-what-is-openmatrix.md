...
The platform runs on **Base**, an Ethereum Layer 2 network, which means lower transaction costs and faster transactions while inheriting Ethereum's security. When an operator configures sponsorship, the platform pays that gas within the operator's policy, which may set a per-identity daily cap; a deployment that sets no cap sponsors without a daily limit. It provides **221 blockchain capabilities** across 21 categories (DeFi, NFTs, identity, governance, social, creator economy, payments, cross-chain, staking & restaking, privacy & ZK, oracles, storage, compute & DePIN, real-world assets, markets, security, and more) -- from token deployment to DAO governance to NFT minting -- all accessible through conversation or API calls.
...
For developers, 0pnMatrx provides a plugin system and SDK that lets you extend the platform, build on top of it, and list your tools in the marketplace. The marketplace lists plugins and does not install them (a plugin runs when its package is placed in `plugins/installed/`); paid plugin sales are not live yet, and their platform commission is an operator setting (the published Terms state 10%).
